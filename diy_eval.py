...
from util.shapenet import ShapeNet, category_ids, DiyShapeNet
from util.datasets import split_list, load_list_from_txt
import models_ae
import mcubes
import trimesh
...
def from_occ_to_pc(occ_points, scale):
    volume_in = np.array(np.where(occ_points > 0)).T / scale
    volume_in = (volume_in - 0.5) / 0.5
    volume_out = np.array(np.where(occ_points < 0)).T / scale
    volume_out = (volume_out - 0.5) / 0.5
...
def main():
    ...
    grid_list = torch.split(grid, split_size**3, dim=1)

    val_crowns_name = load_list_from_txt(os.path.join(args.data_path, 'val.txt'))
    create_colored_pointcloud = partial(trimesh.PointCloud, colors=[255, 255, 0, 1])
    ...
    with torch.no_grad(): 
        ...
        dataset_test = DiyShapeNet(args.data_path, split='test', crowns_list = val_crowns_name, transform=None, sampling=False, return_surface=True, surface_sampling=False)
        ...
        for batch in metric_logger.log_every(data_loader_test, 10, header):
            ...
            # output the reconstructed crown
            output = []
            for one_grid in grid_list:
                output.append(model(surface2048, one_grid)['logits'])
            ...
            if num % save_interval == 0:
                # volume_in, volume_out = from_occ_to_pc(volume, density+1)
                # volume_in = create_colored_pointcloud(volume_in)
                # volume_out = create_colored_pointcloud(volume_out)
                m.export(f'output/eval/crown_{num // save_interval}.obj')
                # volume_in.export(f'output/eval/in_points_{num // save_interval}.obj')
                # volume_out.export(f'output/eval/out_points_{num // save_interval}.obj')
            ...
            print(f'The {num} of crown is finished')

        print(metric_logger.iou.avg, metric_logger.cd.avg, metric_logger.fscore.avg)
